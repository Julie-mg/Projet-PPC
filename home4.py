...
class Home(Process):
    def __init__(self, id, mq_offre, mq_demande, barrier_init, barrier_while, barrier_act, lock, shared_temperature, nb_days, day):
        super().__init__()
        self.conso = random.randrange(6,60)
        self.id = id
        self.trade_policy = random.randint(1,3)
        self.prod = random.randrange(0,60)
...
if __name__ == "__main__":
    ...
    home_processes = [Home(i+1, mq_give, mq_ask, barrier_init, barrier_boucle, barrier_act, lock, shared_memory, nb_days, barrier_day) for i in range(num_homes)]
